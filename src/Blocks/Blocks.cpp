...
namespace voxel
{
	...
	void Blocks::init()
	{
		...
		blocks[6] = (new Block(6))->setDefaultName("sapling")->setHardness(0)->setTex(15. / 16, 0)->setFullCube(false)->setOpacity(0)->setSolid(false)->setLayer(1)->setTransparent(true)->setReplaceable(true)->setAABB(AABB(glm::vec3(.1, 0, .1), glm::vec3(.9, .8, .9)));
		...
		//
		blocks[37] = (new Block(37))->setDefaultName("yellow_flower")->setHardness(0)->setTex(13. / 16, 0)->setFullCube(false)->setOpacity(0)->setSolid(false)->setLayer(1)->setTransparent(true)->setReplaceable(true)->setAABB(AABB(glm::vec3(.3, 0, .3), glm::vec3(.7, .6, .7)));
		blocks[38] = (new Block(38))->setDefaultName("red_flower")->setHardness(0)->setTex(12. / 16, 0)->setFullCube(false)->setOpacity(0)->setSolid(false)->setLayer(1)->setTransparent(true)->setReplaceable(true)->setAABB(AABB(glm::vec3(.3, 0, .3), glm::vec3(.7, .6, .7)));
		blocks[39] = (new Block(39))->setDefaultName("brown_mushroom")->setHardness(0)->setTex(13. / 16, 1. / 16)->setFullCube(false)->setOpacity(0)->setSolid(false)->setLayer(1)->setTransparent(true)->setReplaceable(true)->setAABB(AABB(glm::vec3(.3 ,0, .3), glm::vec3(.7, .4, .7)));
		blocks[40] = (new Block(40))->setDefaultName("red_mushroom")->setHardness(0)->setTex(12. / 16, 1. / 16)->setFullCube(false)->setOpacity(0)->setSolid(false)->setLayer(1)->setTransparent(true)->setReplaceable(true)->setAABB(AABB(glm::vec3(.3 ,0, .3), glm::vec3(.7, .4, .7)));
